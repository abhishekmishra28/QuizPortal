...
export const searchExams = async(query) => {
    try {
        const response = await axiosInstance.get(`/api/exams/searchExams?q=${encodeURIComponent(query)}`)
        return response.data
    }
    catch (error) {
        return error.response.data
...
export const deleteQuestionFromExam = async(id, payload) => {
    try {
        const response = await axiosInstance.delete(`/api/exams/deleteQuestionFromExam/${id}`, { data: payload })
        return response.data
    }
    catch (error) {
        return error.response.data
